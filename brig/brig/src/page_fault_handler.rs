...
//     // let pc = device.well_known_registers.pc();
//     // log::debug!("PC = {pc:016x}");

//     let mmu_enabled = device.register_file.read::<u64>("SCTLR_EL1_bits") & 1
// == 1;

//     // correct the address as it was masked off in
// emitter.rs:read/write-memory     let unmasked_address =
// VirtAddr::new((((faulting_address.as_u64() as i64) << 24) >> 24) as u64);

//     let guest_physical = if mmu_enabled {
//         // translate:
//         // * walk guest page tables from top level page table translate
// faulting address         // * if it doesnt exist: guest page fault
//         // * if it does exist but is invalid (write to a read only mapped
// page)         // * or it works, we get a guest physical address, we do the
// next logic on line         //   186 and map it as writeable, but if it was a
// read then map as read only         // * map that guest physical address into
// the correct location in host virtual         //   memory
...
//     // gp = guest_physical
//     let host_virtual_in_gp_mapping = (GUEST_PHYSICAL_START +
// guest_physical).align_down(0x1000u64);

//     // if tracing::ENABLED.load(core::sync::atomic::Ordering::Relaxed) {
//     //     log::error!(
//     //         "[{:x}] mapped {unmasked_address:p} to
// {host_virtual_in_gp_mapping:p}     // ({})",
// device.register_file.read::<u64>("_TTBR0_EL1_bits"     // ),         if
// is_write { "WRITE" } else { "READ" },     //     )
//     // }
...
//             // No existing backing page, so lookup what to do.
//             if let Some(rgn) = addrspace.find_region(guest_physical) {
//                 // Physical address lies within a valid guest region,
// determine region                 // type...
//                 match rgn.kind() {
//                     AddressSpaceRegionKind::Ram => {
//                         // // Physical address lies within a RAM-backed
// region, so allocate                         // a // backing page.
//                         // let backing_page = VirtAddr::from_ptr(unsafe {
//                         //     alloc_zeroed(Layout::from_size_align(0x1000,
//                         // 0x1000).unwrap()) })
//                         // .to_phys();

//                         // // Map the allocated backing page into the 1-1
// guest phyical                         // memory // area
//                         // VirtualMemoryArea::current().map_page(
//                         //     Page::<Size4KiB>::from_start_address(
...
//                         // log::debug!(
//                         //     "allocated backing page {backing_page:x?} ->
// {:x?}",                         //     (GUEST_PHYSICAL_START +
// guest_physical).align_down(0x1000u64)                         // );
...
//                     }
//                     AddressSpaceRegionKind::IO(device) => {
//                         log::debug!("guest device page fault at rip {:x}",
// machine_context.rip);

//                         let offset = guest_physical - rgn.base();

//                         let data = unsafe { &*(machine_context.rip as *const
// [u8; 15]) };

//                         let mut decoder = iced_x86::Decoder::new(64, data,
// 0);                         let faulting_instruction = decoder.decode();

//                         if is_write {
//                             log::debug!(
//                                 "device write @ {offset:x} with instr
// {faulting_instruction:?}"                             );

//                             let (value, size) = match
// faulting_instruction.op1_kind() {
// OpKind::Register => match faulting_instruction.op1_register() {
// Register::CL => (machine_context.rcx, 1),
// Register::CX => (machine_context.rcx, 2),
// Register::ECX => (machine_context.rcx, 4),
...
// }                                 OpKind::Immediate16 => {
//                                     (faulting_instruction.immediate16() as
// u64, 2)                                 }
//                                 OpKind::Immediate32 => {
//                                     (faulting_instruction.immediate32() as
// u64, 4)                                 }
//                                 OpKind::Immediate64 =>
...
//                                     (faulting_instruction.immediate8to32() as
// u64, 4)                                 }
//                                 OpKind::Immediate8to64 => {
//                                     (faulting_instruction.immediate8to64() as
// u64, 8)                                 }
//                                 OpKind::Immediate32to64 => {
//                                     (faulting_instruction.immediate32to64()
// as u64, 8)                                 }

//                                 kind => {
//                                     panic!(
//                                         "device write todo op1 kind {kind:?}
// {faulting_instruction:?}"                                     )
//                                 }
//                             };

//                             // let (value, size) = match
// faulting_instruction.code() {                             //
// Code::Mov_rm8_r8 => match src {                             //
// Register::CL => (machine_context.rcx, 1),                             //
// reg => {                             //             panic!("todo write src
// reg {reg:?}")                             //         }
//                             //     },
//                             //    => {
...
//                             log::debug!("writing {bytes:x?} to device @
// {offset:x?}");
...
//                             let (dest, size) = match
// faulting_instruction.code() {
// Code::Mov_r32_rm32 => {                                     let dest =
// faulting_instruction.op0_register();
...
//                             log::debug!("read {bytes:x?} from device, writing
// to {dest:?}");
...
//                                     // set data (will zero top half of rax
// when we cast data                                     // to a u64)
//                                     machine_context.rax = data as u64;
//                                 }
//                                 Register::AX => {
//                                     let data =
...
// {bytes:?}, instr: {faulting_instruction:?}"
// )                                 }
//                             }
//                         }
...
//                         machine_context.rip = current_ip +
// faulting_instruction.len() as u64;

//                         log::debug!(
//                             "setting correct return point: current_ip:
// {current_ip:x}, len: {len:x}, new_rip: {:x}",
// machine_context.rip                         );
...
//                 // Physical address not in valid guest region -- real fault.
//                 panic!(
//                     "GUEST PAGE FAULT code {error_code:?} @
// {guest_physical:x?}: no region -- this is a real fault, RIP: {:x}",
//                     machine_context.rip
//                 )
//             }
//         }
...
//     VirtualMemoryArea::current().map_page_propagate_invalidation(
//         Page::<Size4KiB>::from_start_address(faulting_address.
// align_down(0x1000u64)).unwrap(),
//         PhysFrame::from_start_address(backing_page).unwrap(),
//         flags,
//     );
// }
