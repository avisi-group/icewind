use {
    crate::boom::{self, Expression, bits_to_int, passes::destruct_composites},
    common::{
        arena::{Arena, Ref},
        hashmap::HashMap,
        id::Id,
...
impl<'ctx: 'fn_ctx, 'fn_ctx> BlockBuildContext<'ctx, 'fn_ctx> {
    ...
    fn build_specialized_function(
        ...
        match name.as_ref() {
            ...
            // todo: this is really gnarly, replace with a pattern detecting the bit slicing of two
            // concatenated operands (128 bit) and replace with 64-bit logic
            //
            // execute_aarch64_instrs_integer_ins_ext_extract_immediate
            // (d: i64, datasize: i64, lsb: i64, m: i64, n: i64)
            // {
            ...
            "execute_aarch64_instrs_integer_ins_ext_extract_immediate" => {
                ...
                // EEEE_FFFF_1111_2222
                //
                // => AAAABBBBCCCCDDDDEEEEFFFF11119999
                //
                //        |    <- 64 ->    |  <- lsb ->
                // => AAAA|BBBBCCCCDDDDEEEE|FFFF11119999
                //
                // => BBBBCCCCDDDDEEEE

                // AAAA_BBBB_CCCC_DDDD << 16 = BBBB_CCCC_DDDD_0000
                // EEEE_FFFF_1111_2222 >> 48 == EEEE
                ...
                let _1 = build(
                    self.block,
                    self.block_arena_mut(),
                    Statement::Constant(Constant::UnsignedInteger { value: 1, width: 1 }),
                );
                let _64 = build(
                    self.block,
                    self.block_arena_mut(),
                    Statement::Constant(Constant::SignedInteger {
                        value: 64,
                        width: 64,
                    }),
                );
